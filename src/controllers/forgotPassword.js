...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (!emailRegex.test(user.username)) {
      return res.status(200).send('Username must be an email. Please contact support!')
    }
    ...
    return res.status(200).send('An email has been sent with the reset password link. The link will expire in 15 mins.')
  } catch (e) {
    console.log(e);
